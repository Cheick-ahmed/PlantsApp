	...
	<div>
		<form class="font-rubik" @submit.prevent="createPlant">
			...
			<div class="w-full mb-8">
				...
				<input type="text" v-model="form.n_vernaculaire"
				class="block mb-2 w-full shadow rounded p-3 text-sm text-gray-700 focus:shadow-lg transition duration-500"
			...
			<div class="w-full mb-8">
				...
				<input type="text" v-model="form.ng_latin"
			...
			<div class="mb-8">
				<label for="ng_latin" class="flex flex-no-wrap items-end block mb-2 text-gray-800 text-xs uppercase font-medium">
...
<script>
	export default {
		...
		methods : {
			async createPlant() {
				try {
					await this.$axios.post('plants', this.form)
					this.$router.push({
						name : 'admin-plants'
					})
				} catch (e) {
